...
export default function CorteXModal({
  ...
  // Simple tabs: Arqueo or Copiar Lista
  ...
  // Stored branch initial cash fund
  ...
  const handleOpenCloseShiftDialog = () => {
    const defaultLeftFund = Math.min(1000, Math.max(0, expectedCashInDrawer));
  ...
  const handleFinalizeShift = (fundLeft: number, notes: string, printTicket: boolean) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-xs p-2 sm:p-4 overflow-y-auto">
      ...
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-4xl overflow-hidden my-auto animate-in fade-in zoom-in-95 duration-150 flex flex-col max-h-[95vh]">
        ...
        <div className="bg-slate-900 text-white border-b border-slate-800 shrink-0 no-print">
          ...
          {/* Simple Tab Bar */}
          <div className="flex items-center px-4 pt-1 gap-2 bg-slate-950 text-xs">
            <button
              ...
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-2xs divide-y divide-slate-100">
                ...
                <div>
                  ...
                  {expandedCategories.abonos && (
                    <div className="px-4 pb-3 pt-1 bg-slate-50/50 space-y-1.5 border-t border-slate-100">
                      {categoryItems.abonos.length === 0 ? (
                        <p className="text-xs text-slate-400 italic py-1">Sin abonos registrados.</p>
                      ) : (
                        categoryItems.abonos.map((grp, idx) => (
                          <div key={idx} className="flex justify-between text-xs py-1 border-b border-slate-100 last:border-none">
                            <span className="text-slate-700 font-medium">{grp.count}x {grp.name}</span>
                            <span className="font-mono font-bold text-slate-900">${grp.total.toFixed(2)}</span>
                          </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3.5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-black shadow-md cursor-pointer"
                    >
        ...
        <div className="p-3.5 bg-slate-900 text-white flex items-center justify-between gap-2 shrink-0 no-print border-t border-slate-800">
          <button
            type="button"
            onClick={handlePrintThermal}
            className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-cyan-300 border border-slate-700 text-xs font-black rounded-xl shadow-xs transition-all cursor-pointer"
          ...
          {!isHistoric && (
            <button
              ...
              className="flex items-center gap-2 px-5 py-2 bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 hover:from-blue-500 hover:to-indigo-500 text-white text-xs font-black rounded-xl shadow-lg transition-all cursor-pointer"
            >
      ...
      {/* =================================================================================== */}
      {/* 58mm PRINTABLE THERMAL RECEIPT CONTAINER (Visible only in @media print) */}
      {/* =================================================================================== */}
      <div 
          ...
          {isHistoric && existingCorteRecord.cashFundLeftForNextShift !== undefined && (
            <>
              <div className="flex justify-between text-[9px] pt-1">
                ...
                <span className="font-bold">${existingCorteRecord.cashFundLeftForNextShift.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-[9px]">
                <span>Efectivo a Entregar:</span>
                <span className="font-bold">${(existingCorteRecord.cashWithdrawn ?? 0).toFixed(2)}</span>
              </div>
            </>
          )}
        </div>
